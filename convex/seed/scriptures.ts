/**
 * Seed script for scriptures
 * 
 * Usage:
 *   npx convex run seed:seedScriptures
 */

import { mutation } from "../_generated/server";

export const seedScriptures = mutation({
  handler: async (ctx) => {
    // Check if already seeded
    const existing = await ctx.db.query("scriptures").first();
    if (existing) {
    ...
    console.log(`Seeded ${scripturesData.length} scriptures`);
  },
});
